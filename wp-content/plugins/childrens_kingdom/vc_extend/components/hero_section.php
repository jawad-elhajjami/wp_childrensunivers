...
if(!function_exists('hero_section')){
    function hero_section_output($atts, $content = null){
        
        $link = vc_build_link($atts['cta_link']);
        $url = $link['url'];
        $img = wp_get_attachment_image_url($atts['background'], 'full');

        $return = "<section id='hero_section' style='background:url($img)'>";
        
            $return .= "<div class='dark_overlay'></div>";
            $return .= "<div class='hero_section_content'>";

                $return .= "<h1>".$atts['title']."</h1>";
                $return .= "<p class='mb-40'>".$atts['desc']."</p>";
                $return .= '<a href='.$url.' class="ck_button"><svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    add_shortcode('hero_section', 'hero_section_output');
}
