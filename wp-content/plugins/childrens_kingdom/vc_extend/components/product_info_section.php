...
vc_map(
    array(
        ...
        'params' => array(
            ...
            array(
                'type' => 'attach_image',
                'heading' => esc_html__('Featured image', 'childrens_kingdom'),
                'param_name' => 'featured_image'
            ),
            array(
                'type' => 'param_group',
                'heading' => esc_html__('Product feature', 'childrens_kingdom'),
                'param_name' => 'product_features',
                'params' => array(
                    array(
                        'type' => 'textfield',
                        'name' => esc_html__('feature title','childrens_kingdom'),
                        'param_name' => 'feature'
                    )
                )
            ),
...
if(!function_exists('product_info_section')){
    function product_info_section_output($atts, $content = null){

        $link = vc_build_link($atts['cta_link']);
        $img = wp_get_attachment_image_url($atts['featured_image'], 'full');
        $features = vc_param_group_parse_atts($atts['product_features']);
        $url = $link['url'];

        $return = "<section id='prodcut_info_section'>";

            $return .= "<div class='image_container' data-aos='fade-right' data-aos-offset='500'
            data-aos-duration='500'>";
                $return .= "<img src='$img'>";
                $return .= "<div class='grayBox'></div>";
                $return .= "<div class='redBox'></div>";
            $return .= "</div>";

            $return .= "<div class='product_features' data-aos='fade-left' data-aos-offset='500'
            data-aos-duration='500'>";
                $return .= "<h2>".$atts['top_title']."</h2>";
                $return .= "<ul class='features_list'>";
                foreach($features as $f){
                    $return .= "<li data-aos='fade-bottom' class='worksans-font opacity-70'>".$f['feature']."</li>";
                }
                $return .= "</ul>";
    ...
    add_shortcode('product_info_section', 'product_info_section_output');
}
